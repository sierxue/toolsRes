...
\extrainfo{本教程基于已经配置好的虚拟机，介绍科学研究中常用开源工具。
    最主要的目标是想让读者能在短时间内学会用{\rm \LaTeX{}} 写作，
    以及用~{\rm \lstinline{Git}} 工具进行版本控制和团队协作。
...
写作这个教程的时候，版本是 6.1.0.
Virtualbox 虚拟机的安装分为三个步骤：
\begin{enumerate}
    \item 安装基本软件；
    \item 安装 \lstinline{Extension pack};
    \item 安装 \lstinline{Guest additions}.
\end{enumerate}
\subsubsection{Linux 系统}%
\label{ssub:vbox-install-linux}
    ...
    \item Linux 的所有发行版可以参考
        ...
        \begin{itemize}
            ...
            \item 参考
                ...
                \textbf{温馨提示}：当版本跳跃时，比如从 6.0 升级到 6.1,
                \lstinline{sudo apt update} 并不提醒升级，需要运行命令
...
\end{enumerate}
% \begin{tip}\label{note:vbox-download-extpack}
%     安装第二步中的 \lstinline{Extension pack} 软件对 Windows, Mac, Linux
%     等都通用。
%     点击
...
%     双击按提示安装。
% \end{tip}
% \begin{tip}\label{note:vbox-guest-addition}
%     安装第三步中需要的 \lstinline{VboxGuestAdditions.iso},
%     不用手动下载，第一步完成后就已自动下载了。
% \end{tip}

% \subsubsection{Windowns 系统}%
% \label{ssub:vbox-install-win}
%
% 安装分为两步：
% \begin{enumerate}
%     \item 下载
...
%     \item 双击安装
%         \lstinline{Oracle_VM_VirtualBox_Extension_Pack-6.1.0.vbox-extpack};
%     % \item \textbf{温馨提示}: Mac 系统 \textbf{不需要}~安装 \lstinline{Guest additions}.
% \end{enumerate}
%
% \subsubsection{Linux 系统}%
% \label{ssub:vbox-install-linux}
...
\label{ssub:vbox-tip-share-folder}

当虚拟机需要与主机系统交换文件时，就可以设定共享目录。
共享目录的设定详情参见附录 \ref{ssub:vbox-tip-share-folder-a}.

\subsubsection{调整虚拟机窗口大小}%
\label{ssub:vbox-tip-window-size}
...
\label{sssub:vbox-guest-ganx-conf}
...
\label{sssub:vbox-guest-ganx-conf-update-notification}
...
\begin{enumerate}
    ...
    \item 参考 \ref{ssub:vbox-tip-share-folder},
...
\ref{sssub:vbox-guest-ganx-conf} 中的两个 git 仓库控制，请勿修改。
...
\begin{itemize}
    ...
    \item \LaTeX{} 教程：
        \begin{itemize}
            % \item \href{https://github.com/luong-komorebi/Begin-Latex-in-minutes/blob/master/Translation-Chinese.md}
...
\label{ssub:vbox-tip-share-folder-a}
...
\begin{enumerate}
    \item 最大化虚拟机窗口（参见
        \ref{ssub:vbox-tip-window-size}）后打开文件，
        若能见光标就停止，否则进入下一步。
