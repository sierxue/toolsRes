...
\title{开源写作工具快速上手指南}
\subtitle{写作排版利器 \LaTeX{} 和团队协作神器 Git}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=1.0\textwidth]%
  {media/screenshots/learn-latex/pdflatex-error.png}
  \caption{输入中文后的编译错误}%
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=1.0\textwidth]%
  {media/screenshots/learn-latex/pdflatex-error-output.png}
...
\begin{exercise}\label{ex:basic-02-math-c}
    在 \href{learn-latex/basic-02/math-b.tex}{文件}
    \directory{/Documents/learn-latex/basic-02/math-b.tex}
    中按提示输入相应的内容后，将文件保存为 \texttt{math-c.tex}.
    编译此文件, 观察 pdf 输出。
\end{exercise}

要完成练习~\ref{ex:basic-02-math-c}, 我们需要输入以下内容：
\begin{lstlisting}[style=lst]
\begin{equation}
    1 + 2 = 3.
\end{equation}
...
需要输入的位置。
接着输入 \(1 + 2 = 3\) 就完成练习~\ref{ex:basic-02-math-c} 了。
完成此练习的演示可以参见
\href{media/screencasts/math-c.mp4}{录屏文件}
\directory{/media/screencasts/math-c.mp4}.

查看 pdf 输出，我们会看到公式的右侧有一个编号 (1).
这个编号是自动生成的，并且会根据这个公式在正文中的位置，自动变化。
比如，我们如果在这个公式之前插入一个公式，那么这个公式的编号就会变成 (2).

\begin{exercise}\label{ex:basic-02-math-d}
    在 \href{learn-latex/basic-02/math-c.tex}{文件}
    \directory{/Documents/learn-latex/basic-02/math-c.tex}
    中的公式 (1) 的前面插入带编号的公式 \(1 + 1 = 2\),
    将这个文件另存为 \texttt{math-d.tex}.
    编译此文件，观察 pdf 输出。
\end{exercise}
\noindent 详情参见 \href{media/screencasts/math-d.mp4}{录屏文件}
\directory{/media/screencasts/math-d.mp4}.
...
\begin{exercise}\label{ex:basic-02-math-e}
    在 \href{learn-latex/basic-02/math-e.tex}{文件}
    \directory{/Documents/learn-latex/basic-02/math-e.tex}
