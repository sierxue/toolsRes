...
\label{sub:virtualbox-tip-window-size}
...
\subsubsection{更新配置文件 --- 自动方式}%
\label{ssub:update-dotfiles-crontab}

\begin{enumerate}
    \item 将 \ref{ssub:update-dotfiles-manual} 中的所有命令写入
...
\subsection{Gvim 的个性化设置}%
\label{sub:vim-gui-config-customized}
...
例如，在 \lstinline{guifont} 中，有建议在中文系统下，使用如下配置：
\begin{lstlisting}[escapeinside=``]
if has("gui_gtk3")
...
\section{故障排除 (Troubleshooting)}%
\label{sec:vim-troubleshooting}

\subsection{Gvim 打开文件后不见光标}%
\label{sub:vim-ts-cursor}

\begin{enumerate}
    \item 最大化虚拟机窗口（参见
        \ref{sub:virtualbox-tip-window-size}）后打开文件，
        若能见光标就停止，否则进入下一步。
