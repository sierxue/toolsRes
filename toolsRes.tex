...
按第 4 行的提示输入 ``Hello, world!'' 后，同时按下组合键 \keys{ctrl + s},
程序就会编译这个 \LaTeX\ 文档，
...
\begin{latex}\label{tex:command}
    ...
    开头，如模板~\ref{basic-01-text-en} 中第 1 行
...
\hyperlink{books/lshort-zh-cn.pdf.15}%
...
\begin{latex}\label{tex:documentclass}
    \LaTeX\ 源代码以一个 \lstinline{\documentclass} 命令作为开头，
    它指定了文档使用的\emph{文档类}。
\end{latex}

文档类的详情可以参见 \hyperlink{books/lshort-zh-cn.pdf.16}%
...
\begin{exercise}\label{ex:basic-01-text}
    ...
    中按提示输入相应的文字后，按下组合键 \keys{ctrl + s}, 观察 pdf 输出。
\end{exercise}
...
\includepdf[pages=15-16,link=true]{books/lshort-zh-cn.pdf}
